...
class PromptRawDataset(object):
    ...
    def __init__(self, output_path, seed, local_rank, dataset_name):
        ...
        if not dataset_name.startswith('local/jsonfile'):
...
class LocalJsonFileDataset(PromptRawDataset):
    ...
    def __init__(self, output_path, seed, local_rank, dataset_name):
        ...
        subdir = dataset_name[len('local/jsonfile/'):]
        # self.raw_datasets = dict(
        #     train=load_dataset('json', data_files='{}/train.json'.format(subdir)),
        #     eval=load_dataset('json', data_files='{}/eval.json'.format(subdir))
        # )
        self.raw_datasets = load_dataset('json', data_files={
            "train": f'{subdir}/train.json',
            "eval": f'{subdir}/train.json'
        })
    ...
    def get_chosen(self, sample):
        if sample['answer'] is not None:
            return " Assistant: " + sample['answer']
        return None
    ...
    def get_prompt_and_chosen(self, sample):
        if sample['input'] is not None and sample['answer'] is not None:
            return " Human: " + sample['input'] + " Assistant: " + sample['answer']
        return None

    def get_prompt_and_rejected(self, sample):
        if sample['input'] is not None and sample['rejected'] is not None:
            return " " + sample['input'] + " " + sample['rejected']
        return None
...
# English dataset
class PKUSafeRLHFDataset(PromptRawDataset):
    """
    https://huggingface.co/datasets/PKU-Alignment/PKU-SafeRLHF/viewer/PKU-Alignment--PKU-SafeRLHF/train?row=2
    """

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "PKU-Alignment/PKU-SafeRLHF"
        self.dataset_name_clean = "pku_safeRLHF"
    ...
    def get_chosen(self, sample):
        # If both are safe, return the safer one.
        if sample["is_response_0_safe"] and sample["is_response_1_safe"]:
            safer_id = sample["safer_response_id"]
            return sample["response_%d" % safer_id]
        # Only response 0 is safe.
        elif sample["is_response_0_safe"] and not sample["is_response_1_safe"]:
            return sample["response_0"]
        # Only response 1 is safe.
        elif not sample["is_response_0_safe"] and sample["is_response_1_safe"]:
    ...
    def get_rejected(self, sample):
        # If both are unsafe, return the NOT safer one.
        if not sample["is_response_0_safe"] and not sample["is_response_1_safe"]:
            safer_id = sample["safer_response_id"]
            return sample["response_%d" % (1 - safer_id)]
        # Response 0 is unsafe.
        elif not sample["is_response_0_safe"] and sample["is_response_1_safe"]:
            return sample["response_0"]
        # Response 1 is unsafe.
        elif sample["is_response_0_safe"] and not sample["is_response_1_safe"]:
            return sample["response_1"]
        # Neither is unsafe.
        else:
