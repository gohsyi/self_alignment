...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
    ...
    if "Dahoas/rm-static" in dataset_name:
        ...
        return raw_datasets.LmqgQagjaquadDataset(output_path, seed, local_rank,
                                                 dataset_name)
    elif "PKU-Alignment/PKU-SafeRLHF" in dataset_name:
        return raw_datasets.PKUSafeRLHFDataset(
            output_path, seed, local_rank, dataset_name
        )
    elif "local/jsonfile" in dataset_name:
        dir = dataset_name[len('local/jsonfile/'):]
        if not (os.path.isfile(f'{dir}/train.json')):
            raise RuntimeError(
                f"Please check both the {dir}/train.json "
                f"and {dir}/eval.json files in your data directory."
            )
        return raw_datasets.LocalJsonFileDataset(output_path, seed, local_rank, dataset_name)
    else:
        raise RuntimeError(
            f"We do not have configs for dataset {dataset_name}, "
            "but you can add it by yourself in raw_datasets.py."
        )
...
def create_dataset(local_rank, dataset_name, data_split, output_path,
                   train_phase, seed, tokenizer, end_of_conversation_token, max_seq_len):
