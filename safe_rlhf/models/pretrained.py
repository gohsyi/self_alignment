...
def resize_tokenizer_embedding(tokenizer: PreTrainedTokenizerBase, model: PreTrainedModel) -> None:
    ...
    if tokenizer.pad_token is None:
        print('Setting pad_token to {}'.format(DEFAULT_PAD_TOKEN))
        special_tokens_dict['pad_token'] = DEFAULT_PAD_TOKEN
    if tokenizer.eos_token is None:
        print('Setting eos_token to {}'.format(DEFAULT_EOS_TOKEN))
        special_tokens_dict['eos_token'] = DEFAULT_EOS_TOKEN
    if tokenizer.bos_token is None:
        print('Setting bos_token to {}'.format(DEFAULT_BOS_TOKEN))
        special_tokens_dict['bos_token'] = DEFAULT_BOS_TOKEN
    if tokenizer.unk_token is None:
        print('Setting unk_token to {}'.format(DEFAULT_UNK_TOKEN))
    ...
    if num_new_tokens > 0:
        ...
        if not is_model_parallel:
            print('Resizing model embedding matrix...')
...
def load_pretrained_models(
    ...
    *,
    ...
    Args:
        ...
        model_max_length (int, optional): The maximum sequence length of the model. Defaults to 2048.
